...
def url_gen(request):
    urls = {
        'dashboards': request.route_url('dashboards',
                                        resource_id='REPLACE_RID').replace(
            'REPLACE_RID', ':resourceId'),
        'dashboardsNoId': request.route_url('dashboards_no_id'),
        'dashboardsProperty': request.route_url('dashboards_property',
                                                key='REPLACE_KEY',
                                                resource_id='REPLACE_RID').replace(
            'REPLACE_KEY', ':key').replace('REPLACE_RID', ':resourceId'),
        'charts': request.route_url('charts', chart_id='REPLACE_CID').replace(
            'REPLACE_CID', ':chartId'),
        'chartsNoId': request.route_url('charts_no_id'),
        'chartsProperty': request.route_url('charts_property',
                                            key='REPLACE_KEY',
                                            chart_id='REPLACE_CID').replace(
            'REPLACE_KEY', ':key').replace('REPLACE_CID', ':chartId'),
        'chartsEventRules': request.route_url('charts_event_rules',
                                              rule_id='REPLACE_CID').replace(
            'REPLACE_CID', ':ruleId'),
...
def includeme(config):
    ...
    config.add_view('ae_charting_ee.views.dashboard:index',
                    route_name='dashboards_no_id', renderer='json',
                    permission='authenticated', request_method="GET")

    config.add_view('ae_charting_ee.views.dashboard:dashboard_POST',
                    route_name='dashboards_no_id', renderer='json',
                    request_method="POST", permission='create_resources')

    config.add_view('ae_charting_ee.views.dashboard:dashboard_PATCH',
                    route_name='dashboards', renderer='json',
                    request_method="PATCH", permission='edit')

    config.add_view('ae_charting_ee.views.dashboard:dashboard_GET',
                    route_name='dashboards',
                    context_type_class=('resource', Dashboard),
                    renderer='json', request_method="GET", permission='view')

    config.add_view('ae_charting_ee.views.dashboard:dashboard_DELETE',
                    route_name='dashboards',
                    context_type_class=('resource', Dashboard),
                    renderer='json', request_method="DELETE",
                    permission='delete')

    config.add_view('ae_charting_ee.views.dashboard:charts_PATCH',
                    route_name='charts', request_method="PATCH",
                    permission='edit', renderer='json',
                    context_type_class=('resource', Dashboard))

    config.add_view('ae_charting_ee.views.dashboard:charts_data',
                    route_name='charts_property', match_param='key=data',
                    context_type_class=('resource', Dashboard),
    ...
    config.add_view('ae_charting_ee.views.dashboard:charts_event_rules_POST',
                    route_name='charts_event_rules_no_id',
                    context_type_class=('chart', DashboardChart),
                    request_method="POST",
                    renderer='json', permission='view')

    config.add_view('ae_charting_ee.views.dashboard:charts_event_rules_GET',
                    route_name='charts_event_rules_no_id',
                    context_type_class=('chart', DashboardChart),
                    request_method="GET",
                    renderer='json', permission='view')
