...
def build_filter_settings_from_chart_config(
    ...
    additional_parms = dict([(k, v) for k, v in params.items()
                             if k in accepted_search_params and v])
...
def process_agg(request, agg, override_app_ids=None):
    ...
    if not agg.get('computed'):
        ...
        elif agg['type'] in ['stats', 'percentiles', 'min', 'max', 'sum',
                             'avg', 'extended_stats', 'range']:
            agg['config']['field'] = 'tags.%s.numeric_values' % \
                                     fix_dot(agg['config']['field'])
    ...
    # computed field path
        ...
        for field in agg['computed_fields']:
            ...
            try:
                script += ' %s %s' % (field['op'] if field['op'] else '',
                                      float(field['field']))
            except ValueError:
                script += tmpl % (field['op'] if field['op'] else '',
                                  field['field'])
    ...
    agg_filters = build_filter_settings_from_chart_config(
        request, {"filters": agg.get('filters', [])},
        override_app_ids=override_app_ids)
    ...
    if not agg.get('computed'):
        ...
        compute_from = {"script": agg['config']['script'],
                        "lang": "expression"}
    ...
    sub_agg = {
        ...
        "aggs": {
            "sub_agg": {
                agg['type']: compute_from
            }
        }
    }
    if agg.get('deepAggEnabled'):
        agg_id, parsed_agg = process_agg(request, agg['deepAgg'])
        sub_agg['aggs']['sub_agg']['aggs'] = {
            "deep_agg": parsed_agg['aggs']['sub_agg']
        }
...
def traverse_sub_aggs(aggregation_list, data):
    ...
    for agg in aggregation_list:
        if agg['id'] in data:
            ...
            fields.append({"name": agg['field_name'],
                           "agg_type": agg['type'],
                           ...
                           "agg_id": agg['id'],
                           "traversed": traverse_sub_aggs(aggregation_list,
                                                          data[agg['id']])})
    if fields:
        return fields
    else:
...
def save_label(info_dict, agg_id, agg, f_type=None, key=None):
    ...
    if f_type in ['count', 'bucket']:
        label = {"agg": agg['agg_id'], "name": agg['name'],
                 "type": agg['agg_type'], "key": key,
                 'human_label': '<label>'}
        ...
        if agg.get('sub_agg_type'):
            label = {"agg": agg['agg_id'], "name": agg['sub_agg_field'],
                     "type": agg['sub_agg_type'], "key": key,
                     'human_label': '<label>'}

            if agg['sub_agg_config'].get('label'):
                h_label = '%s %s' % (key,
                                     agg['sub_agg_config']['label'])
            else:
                h_label = '{} {} {}'.format(
...
def parse_metrics(parent_agg, info_dict, es_result, counter):
    ...
    if 'doc_count' in es_result:
        es_result = [{"name": parent_agg['config']['field'],
    ...
    for agg in es_result:
        ...
        if "buckets" in traversed:
            ...
            for entry in traversed['buckets']:
                ...
                # names
                subkey, counter = gen_subkey(info_dict, agg['agg_id'],
                                             entry['key'], counter)
                key_id = info_dict['subkeys'][agg['agg_id']][entry['key']]
                field_id = "%s_%s" % (agg['agg_id'], key_id)
                save_label(info_dict, field_id, agg, f_type="bucket",
                           key=entry['key'])
                gen_groups(info_dict, agg, field_id)
            ...
            for k, v in to_iter:
                ...
                save_label(info_dict, field_id, agg, key=k,
                           f_type="nonbucket")
                gen_groups(info_dict, agg, field_id)
...
def parse_es_result(elasticsearch_data, es_config, json_config):
    ...
    result = traverse_sub_aggs(aggregations,
                               elasticsearch_data.get('aggregations', {}))
    single_row = False
    ...
    if parent_agg and parent_agg['type'] == 'time_histogram':
        ...
        for bucket in buckets:
            ...
            item, counter = parse_metrics(parent_agg, info_dict,
                                          traversed, counter)
            item['key'] = t_key
            series.append(item)
        ...
        for bucket in buckets:
            traversed = traverse_sub_aggs(aggregations, bucket)
            item, counter = parse_metrics(
                parent_agg, info_dict, traversed, counter)
            item['key'] = bucket['key']
            series.append(item)
    else:
        single_row = True
        temp_series, counter = parse_metrics(parent_agg, info_dict, result,
                                             counter)
        series = []
        for k, v in temp_series.items():
            row = {
                k: v,
                "key": info_dict['system_labels'][k]
            }
            series.append(row)
...
def transform_json_to_es_config(request, json_body, filter_settings,
                                ids_to_override=None, ixtypes=None):
    """
    ...
    # will be needed for label generation
    if (parent_agg and parent_agg['type'] and
                parent_agg['type'] != 'time_histogram'):
    ...
    for agg in aggregations:
        ...
        filtered_fields = list(filter(float_test,
                                 [f['field'] for f in computed_fields
                                  if f['field']]))
        if agg.get('computed'):
    ...
    if tags_to_check:
        ...
        if index_names:
            mappings = request.es_conn.get_mapping(
                index=index_names, doc_type='log')
        else:
            mappings = {}

        for k, v in mappings.items():
            tags = v['mappings']['log']['properties'].get('tags',
                                                          {'properties': {}})
