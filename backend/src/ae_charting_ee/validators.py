...
time_range_choices = ['1m', '5m', '30m', '1h', '4h', '12h', '24h', '3d',
                      '1w', '2w', '1M', '3M', '6M', '12M']
...
agg_type_choices = ['terms', 'percentiles', 'min', 'max', 'sum', 'avg',
                    'value_count', 'cardinality']

deep_agg_type_choices = ['min', 'max', 'sum', 'avg', 'value_count',
                         'cardinality']
...
class ChartDeepAggSchema(colander.MappingSchema):
    ...
    type = colander.SchemaNode(colander.String(),
                               validator=colander.OneOf(deep_agg_type_choices))
    computed = colander.SchemaNode(colander.Boolean(),
                                   missing=False)
...
class ChartAggSchema(colander.MappingSchema):
    id = colander.SchemaNode(colander.String())
    type = colander.SchemaNode(colander.String(),
                               validator=colander.OneOf(agg_type_choices))
...
class ChartConfigSchema(colander.MappingSchema):
    ...
    resource = colander.SchemaNode(colander.Integer(),
                                   validator=possible_applications_validator)
    datasource = colander.SchemaNode(colander.String(),
                                     missing='logs',
                                     validator=colander.OneOf(['logs',
                                                               'metrics']))
    postProcess = ConfigTypeSchema(missing=None)
    timeRange = colander.SchemaNode(colander.String(),
                                    validator=colander.OneOf(
                                        time_range_choices))
    startMomentValue = colander.SchemaNode(colander.Integer(),
                                           missing=0,
                                           validator=colander.Range(min=0))
    startMoment = colander.SchemaNode(colander.String(),
                                      missing='now',
                                      validator=colander.OneOf(
                                          start_moment_choices))
    startMomentUnit = colander.SchemaNode(colander.String(),
                                          missing='minutes',
                                          validator=colander.OneOf(
                                              start_moment_unit_choices))
    aggs = AggListSchema()
